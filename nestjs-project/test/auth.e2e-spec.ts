...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email, password });
    const body = res.body as TokenPairResponseBody;
    return {
  ...
  describe('GET /auth/confirm-email', () => {
    ...
    it('returns 401 with INVALID_TOKEN on an already-used token', async () => {
      ...
      expect((res.body as ErrorResponseBody).error).toBe('INVALID_TOKEN');
    });
    ...
    it('returns 401 with TOKEN_EXPIRED on an expired token', async () => {
      ...
      expect((res.body as ErrorResponseBody).error).toBe('TOKEN_EXPIRED');
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect((res.body as MeResponseBody).sub).toBeDefined();
      expect((res.body as MeResponseBody).email).toBe('me@example.com');
    });
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 200 with access_token and refresh_token on valid credentials', async () => {
      ...
      expect((res.body as TokenPairResponseBody).access_token).toBeDefined();
      expect((res.body as TokenPairResponseBody).refresh_token).toBeDefined();
      expect(typeof (res.body as TokenPairResponseBody).access_token).toBe(
        'string',
      );
      expect(typeof (res.body as TokenPairResponseBody).refresh_token).toBe(
        'string',
      );
    });
    ...
    it('returns 401 with INVALID_CREDENTIALS on wrong password', async () => {
      ...
      expect((res.body as ErrorResponseBody).error).toBe('INVALID_CREDENTIALS');
    });
    ...
    it('returns 403 with EMAIL_NOT_CONFIRMED when user is not confirmed', async () => {
      ...
      expect((res.body as ErrorResponseBody).error).toBe('EMAIL_NOT_CONFIRMED');
    });
  ...
  describe('POST /auth/refresh', () => {
    it('returns 200 with new access_token and refresh_token on valid refresh token', async () => {
      ...
      expect((res.body as TokenPairResponseBody).access_token).toBeDefined();
      expect((res.body as TokenPairResponseBody).refresh_token).toBeDefined();
      expect((res.body as TokenPairResponseBody).refresh_token).not.toBe(
        refresh_token,
      );
    });
  ...
  describe('POST /auth/logout', () => {
    ...
    it('revokes all refresh tokens so subsequent refresh calls return 401', async () => {
      ...
      expect(['INVALID_TOKEN', 'TOKEN_REUSE_DETECTED']).toContain(
        (res.body as ErrorResponseBody).error,
      );
    });
  });
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect(
        (loginRes.body as TokenPairResponseBody).access_token,
      ).toBeDefined();
    });
